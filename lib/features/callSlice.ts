// Goat-JobHunter-Mobile-FE/lib/features/callSlice.ts
  ...
  reducers: {
    registerCallRealtimeEvent: (
      ...
      action: PayloadAction<
        Omit<CallRealtimeEvent, "occurredAt"> & { occurredAt?: string }
      >,
    ) => {
      const occurredAt = resolveEventTimestamp(action.payload.occurredAt);
      state.lastRealtimeEvent = { ...action.payload, occurredAt };
      state.lastRealtimeEventAt = occurredAt;
    },
    setOutgoingCallPending: (state, action: PayloadAction<CallSession>) => {
      state.currentCall = { ...action.payload, status: CallStatusEnum.PENDING };
    ...
    setCurrentCall: (state, action: PayloadAction<CallSession>) => {
      ...
      state.localVideoEnabled =
        action.payload.callType === CallTypeEnum.VIDEO;
      state.participantMediaStates = {};
    },
    setCallStatus: (
      state,
      action: PayloadAction<{ sessionId: number; status: CallStatusEnum }>,
    ) => {
      if (
        state.currentCall &&
        state.currentCall.sessionId === action.payload.sessionId
      ) {
    ...
    endCurrentCall: (
      ...
      if (targetSessionId) {
        const isCurrentCallTarget = isMatchingCall(
          state.currentCall,
          targetSessionId,
        );
        const isIncomingCallTarget =
          state.incomingCall?.sessionId === targetSessionId;
      ...
      if (state.currentCall) {
        state.currentCall.status =
          action.payload?.finalStatus ?? CallStatusEnum.ENDED;
        state.currentCall.endedAt =
          action.payload?.endedAt ?? new Date().toISOString();
      }
    ...
    setRtcConnectionState: (
      ...
      if (
        ...
        !isMatchingCall(state.currentCall, action.payload.sessionId)
      ) {
        return;
      }
    ...
    setLocalAudioEnabled: (
      state,
      action: PayloadAction<{ sessionId?: number; enabled: boolean }>,
    ) => {
      if (
        action.payload.sessionId &&
        !isMatchingCall(state.currentCall, action.payload.sessionId)
...
const selectCallState = (state: { call: CallState }) => state.call;
...
export const selectCallError = createSelector(
  selectCallState,
  (state) => state.callError,
);
export const selectLastCallRealtimeEvent = createSelector(
  selectCallState,
  (state) => state.lastRealtimeEvent,
);
export const selectLastCallRealtimeEventAt = createSelector(
  selectCallState,
  (state) => state.lastRealtimeEventAt,
);
export const selectRtcConnectionState = createSelector(
  selectCallState,
  (state) => state.rtcConnectionState,
);
export const selectLocalAudioEnabled = createSelector(
  selectCallState,
  (state) => state.localAudioEnabled,
);
export const selectLocalVideoEnabled = createSelector(
  selectCallState,
  (state) => state.localVideoEnabled,
);
export const selectRemoteAudioActive = createSelector(
  selectCallState,
  (state) => state.remoteAudioActive,
);
export const selectRemoteVideoActive = createSelector(
  selectCallState,
  (state) => state.remoteVideoActive,
);
export const selectParticipantMediaStates = createSelector(
  selectCallState,
  (state) => state.participantMediaStates,
);
export const selectIsCallActive = createSelector(
  selectCallState,
  (state) =>
    state.currentCall !== null &&
    (state.currentCall.status === CallStatusEnum.PENDING ||
      state.currentCall.status === CallStatusEnum.ACTIVE),
);

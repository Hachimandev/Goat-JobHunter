...
import { useAddMemberToGroupMutation } from "@/services/chatRoom/groupChat/groupChatApi";
...
export const AddmembersScreen = () => {
  ...
  const existingMembers = members ? JSON.parse(members) : [];

  const [addMembers, { isLoading: isAdding }] = useAddMemberToGroupMutation();
  ...
  const handleAdd = async () => {
    ...
    try {
      for (const user of selectedUsers) {
        await addMembers({
          chatroomId: String(parsedGroupId),
  ...
  const renderSearchResult = ({ item }: { item: User }) => {
    ...
    const isAlreadyInGroup = existingMembers.some(
      (m: any) => m.accountId === item.accountId,
    );

    return (
      <TouchableOpacity
        style={[styles.userItem, isAlreadyInGroup && { opacity: 0.4 }]}
        onPress={() => {
          if (!isAlreadyInGroup) {

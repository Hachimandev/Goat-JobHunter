...
public class RedisKeyExpirationListener implements MessageListener {
    ...
    public void onMessage(Message message, byte[] pattern) {
        ...
        try {
            ...
            String payload = this.redisService.getValue(dataKey);
            ...
            Map<String, Object> data = this.objectMapper.readValue(payload, Map.class);

            // Build and save notification
            Notification saved = this.notificationService.createNotification(
                    this.notificationService.buildNotification(data)
            );

            // Send via WebSocket
            this.notificationService.sendNotificationToUser(saved.getRecipient(), saved);

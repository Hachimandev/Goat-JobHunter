...
type TypingIndicatorListener = (
  participants: TypingIndicatorParticipant[],
) => void;

const TYPING_EXPIRY_MS = 4500;

const activeTypingByRoom = new Map<
  number,
  Map<number, TypingIndicatorParticipant>
>();
const listenersByRoom = new Map<number, Set<TypingIndicatorListener>>();
const expiryTimersByRoom = new Map<
  number,
  Map<number, ReturnType<typeof setTimeout>>
>();

const getRoomState = (roomId: number) =>
  activeTypingByRoom.get(roomId) ?? new Map<number, TypingIndicatorParticipant>();

const getRoomListeners = (roomId: number) =>
  listenersByRoom.get(roomId) ?? new Set<TypingIndicatorListener>();

const getSnapshot = (roomId: number): TypingIndicatorParticipant[] =>
  Array.from(getRoomState(roomId).values()).sort(
    (left, right) =>
...
const scheduleParticipantExpiry = (roomId: number, accountId: number) => {
  ...
  const roomTimers =
    expiryTimersByRoom.get(roomId) ??
    new Map<number, ReturnType<typeof setTimeout>>();
  roomTimers.set(accountId, timer);
  expiryTimersByRoom.set(roomId, roomTimers);
};

export const upsertTypingIndicator = (
  roomId: number,
  participant: TypingIndicatorParticipant,
) => {
  ...
  if (
    ...
    !participant ||
    !Number.isFinite(participant.accountId)
  ) {
    return;
  }

  const roomState =
    activeTypingByRoom.get(normalizedRoomId) ??
    new Map<number, TypingIndicatorParticipant>();
...
export const getTypingIndicators = (roomId: number) => getSnapshot(roomId);

export const subscribeTypingIndicators = (
  roomId: number,
  listener: TypingIndicatorListener,
) => {
  const normalizedRoomId = Number(roomId);
  ...
  const roomListeners =
    listenersByRoom.get(normalizedRoomId) ??
    new Set<TypingIndicatorListener>();

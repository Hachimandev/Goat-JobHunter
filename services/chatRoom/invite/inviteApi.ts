...
export const inviteApi = api.injectEndpoints({
  endpoints: (builder) => ({
    ...
    toggleInviteLink: builder.mutation<InviteLinkResponse, { roomId: number } & ToggleInviteRequest>({
      query: ({ roomId, ...data }) => ({
    ...
    getPendingJoinRequests: builder.query<GroupJoinRequestListResponse, number>({
      ...
      providesTags: (_, __, roomId) => [{ type: 'ChatInvite', id: `join-requests-${roomId}` }],
    }),
    ...
    approveJoinRequest: builder.mutation<void, { roomId: number; requestId: number }>({
      ...
      invalidatesTags: (_, __, { roomId }) => [{ type: 'ChatInvite', id: `join-requests-${roomId}` }],
    }),
